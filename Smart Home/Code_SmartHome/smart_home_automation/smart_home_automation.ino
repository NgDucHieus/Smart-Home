...
void touchDetect(void *parameter) {
  ...
  while (true) {
    touchValue = digitalRead(touchSensor);  // Read the touch sensor value

    if (touchValue == 1) {  // If the touch sensor is touched (value is 1)
      SerialBT.print("Touch active?");
      Serial.println(touchValue);
      digitalWrite(touchLed, HIGH);     // Turn on the touch LED
      digitalWrite(touchBuzzer, HIGH);  // Turn on the touch Buzzer
      touchStatus = true;               // Set the touch status to true
    }
    else if (touchValue == 0) {  // If the touch sensor is not touched (value is 0)
      SerialBT.print("Touch inactive?");
      Serial.println(touchValue);
      digitalWrite(touchLed, LOW);      // Turn off the touch LED
      digitalWrite(touchBuzzer, LOW);   // Turn off the touch Buzzer
      touchStatus = false;              // Set the touch status to false
    }
    vTaskDelay(1000 / portTICK_PERIOD_MS);
  }
}
